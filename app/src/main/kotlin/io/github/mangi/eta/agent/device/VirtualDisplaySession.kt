...
internal object VirtualDisplaySession {
    ...
    private fun failPreservingPrior(s: Session, code: String): JSONObject {
        ...
        if (code == "RECOVERY_UNCERTAIN" && priorCode.isNotBlank() && priorCode != "RECOVERY_UNCERTAIN") {
            s.phase = "uncertain"
            return prior!!
        }
        return fail(s, code)
    ...
    @Synchronized fun finish(runId: String, context: Context? = null): JSONObject {
        ...
        if (action == VirtualDisplayRecoveryPolicy.Action.HANDOFF && s.kept.isNotEmpty()) {
            ...
            if (!handoff.ok || body(handoff).opt("handedOff") != true || body(handoff).opt("sourceEmpty") != true)
                return fail(s, handoff.errorCode.ifBlank { "HANDOFF_UNCERTAIN" }, safeOwnerDetail(handoff))
        }
