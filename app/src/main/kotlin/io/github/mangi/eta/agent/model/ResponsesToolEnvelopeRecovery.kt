...
internal object ResponsesToolEnvelopeRecovery {
    ...
    private val rejection = Regex(
        "basispoints tool transport code must contain one JSON client-tool envelope; " +
            ...
            "json_offset=[0-9]+;? json_failure=missing[-_]separator\\)",
    )
    ...
    const val SERIALIZATION =
        "This transport accepts only one client-tool envelope per response. Generate one fresh " +
    ...
    const val CORRECTION = "The preceding generation failed the client-tool JSON validator. " + SERIALIZATION

    // Learned only from a guarded, exact rejection. Keep bounded and process-local;
    // never infer restrictions from model names or change all Responses endpoints.
    private val restrictedEndpoints = linkedSetOf<Pair<String, String>>()

    @Synchronized
    fun rememberRestriction(request: ProviderRequest) {
        val key = request.config.baseUrl.trimEnd('/') to request.config.model
        restrictedEndpoints.remove(key)
        restrictedEndpoints.add(key)
        while (restrictedEndpoints.size > 64) restrictedEndpoints.remove(restrictedEndpoints.first())
    }

    @Synchronized
    fun prepare(request: ProviderRequest): ProviderRequest =
        if (!request.singleToolCall && (request.config.baseUrl.trimEnd('/') to request.config.model) in restrictedEndpoints)
            withInstructions(request, SERIALIZATION) else request

    fun corrected(request: ProviderRequest): ProviderRequest = withInstructions(request, CORRECTION)

    private fun withInstructions(request: ProviderRequest, instructions: String): ProviderRequest {
        ...
        messages.put(JSONObject().put("role", "developer").put("content", instructions))
        return request.copy(messages = messages, singleToolCall = true)
    }
